...
class FirebaseAuthentication_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'firebase-authentication'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'firebase-authentication'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'firebase-authentication'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'firebase-authentication'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'firebase-authentication');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'firebase-authentication'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'firebase-authentication');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'firebase-authentication'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'firebase-authentication') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function createFormControl($aOptionKey, $aOptionMeta, $savedOptionValue) {
        ...
        // echo $aOptionKey;
        // print_r($aOptionMeta);
        // echo $savedOptionValue;
        $savedOptionValue = stripcslashes($savedOptionValue);

        if (is_array($aOptionMeta)) {

            if ($aOptionMeta[1] == 'select') {
                 // Drop-down list
                $choices = $aOptionMeta[2]; // array_slice($aOptionMeta, 1);
                ?>
            ...
            } else if ($aOptionMeta[1] == 'textarea') {
                // Simple input field
                ?>
                <p><textarea type="text" name="<?php echo $aOptionKey ?>" id="<?php echo $aOptionKey ?>" cols="50" rows="5"><?php echo esc_attr($savedOptionValue) ?></textarea>
                </p>
                <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'firebase-authentication');
            case 'false':
                return __('false', 'firebase-authentication');

            case 'Administrator':
                return __('Administrator', 'firebase-authentication');
            case 'Editor':
                return __('Editor', 'firebase-authentication');
            case 'Author':
                return __('Author', 'firebase-authentication');
            case 'Contributor':
                return __('Contributor', 'firebase-authentication');
            case 'Subscriber':
                return __('Subscriber', 'firebase-authentication');
            case 'Anyone':
                return __('Anyone', 'firebase-authentication');
        }
        return $optionValue;
    }
